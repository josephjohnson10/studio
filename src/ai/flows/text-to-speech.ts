...
const textToSpeechFlow = ai.defineFlow(
  ...
  async ({ text }) => {
    ...
    if (!media.url || !media.url.startsWith('data:audio/pcm;base64,')) {
      throw new Error('Unexpected audio data format from the model.');
    }
    ...
    if (audioBuffer.length === 0) {
      throw new Error('Received empty audio buffer from the model.');
    }
